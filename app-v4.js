...
function purchaseBillCard(bill){
  ...
  return `<article class="item-card purchase-card"><div class="purchase-card-top"><div><div class="project-card-number">${esc(bill.bill_number)}</div><h3>${esc(supplierName(bill.supplier_id))}</h3></div><div><div class="amount">${money(bill.total_amount)}</div><span class="status ${statusClass(bill.payment_status)}">${esc(bill.payment_status)}</span></div></div><div class="purchase-card-info"><div><span>Bill date</span><b>${prettyDate(bill.bill_date)}</b></div><div><span>Due date</span><b>${prettyDate(bill.due_date)}</b></div><div><span>Amount paid</span><b>${money(bill.amount_paid)}</b></div><div><span>Balance due</span><b>${money(purchaseBalance(bill))}</b></div></div><div class="project-card-actions"><button class="quick-btn view" data-view-purchase-bill="${bill.id}">View bill</button>${purchaseBalance(bill)>0?`<button class="quick-btn receipt" data-pay-purchase-bill="${bill.id}">Pay supplier</button>`:''}</div></article>`
}
...
document.addEventListener('click',e=>{
  ...
  const payBill=e.target.closest('[data-pay-purchase-bill]');if(payBill)openPayment('supplier_payment',{purchaseBillId:payBill.dataset.payPurchaseBill});
