...
function dashboardFinancialValues(){
  const projects=state.projects.filter(p=>p.status==='Approved'),orders=state.walkInOrders.filter(o=>!['Pending','Cancelled'].includes(o.status)),receivable=projects.reduce((sum,p)=>sum+Math.max(0,projectMetrics(p).balance),0)+orders.reduce((sum,o)=>sum+Math.max(0,walkInMetrics(o).balance),0),payable=state.purchaseBills.filter(b=>b.status!=='Cancelled').reduce((sum,b)=>sum+purchaseBalance(b),0),cash=state.paymentAccounts.reduce((sum,a)=>sum+paymentAccountBalance(a),0),jobProfit=projects.reduce((sum,p)=>sum+projectMetrics(p).profit,0)+orders.reduce((sum,o)=>sum+walkInMetrics(o).profit,0),otherIncome=state.payments.filter(p=>p.payment_type==='income').reduce((sum,p)=>sum+Number(p.amount||0),0),expenses=state.payments.filter(p=>p.payment_type==='expense'&&!p.project_id&&!p.walk_in_order_id).reduce((sum,p)=>sum+Number(p.amount||0),0);
...
function applyRolePermissions(){
  ...
  const selectors=['[data-action="new-party"]','[data-action="new-project"]','[data-action="new-invoice"]','[data-action="new-material"]','[data-action="new-purchase"]','[data-action="new-supplier"]','[data-action="new-purchase-bill"]','[data-action="new-cash-purchase"]','[data-action="new-walk-in-order"]','[data-action="new-payment-account"]','[data-action="new-payment"]','[data-action="record-receipt"]','[data-action="record-expense"]','[data-action="add-labourer"]','[data-action="assign-labour"]','[data-action="labour-payment"]','[data-action="labour-advance"]','[data-action="assign-operation-material"]','[data-action="new-expense-category"]','[data-action="new-service"]','[data-action="scope-increase-from-list"]','[data-action="scope-decrease-from-list"]','[data-edit-party]','[data-edit-project-card]','[data-edit-order-card]','[data-edit-material-card]','[data-edit-labourer]','[data-edit-supplier]','[data-edit-expense-category]','[data-edit-service]','[data-edit-payment-account]','[data-convert-project-invoice]','[data-convert-order-invoice]','[data-approve-order-quote]','[data-purchase-material]','[data-pay-purchase-bill]','#editProjectBtn','#approveProjectBtn','#cancelProjectBtn','#scopeIncreaseBtn','#scopeDecreaseBtn','#generateProjectInvoiceBtn','#editMaterialBtn','#purchaseMaterialBtn','#payPurchaseBillBtn','#editWalkInOrderBtn','#advanceWalkInOrderBtn','#cancelWalkInOrderBtn','#editPaymentAccountBtn','#accountMoneyInBtn','#accountMoneyOutBtn','#accountTransferBtn','#editInvoiceBtn','#receiveInvoicePaymentBtn'];selectors.forEach(selector=>$$(selector).forEach(el=>el.classList.toggle('role-hidden',!writable)));
}
