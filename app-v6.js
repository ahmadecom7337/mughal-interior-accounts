...
const paymentTypeLabel=type=>({customer_receipt:'Customer receipt',supplier_payment:'Supplier payment',income:'Other income',expense:'Business expense',transfer:'Account transfer'}[type]||type);
const paymentTypeIcon=type=>({customer_receipt:'↙',supplier_payment:'↗',income:'+',expense:'−',transfer:'⇄'}[type]||'•');
...
function renderPayments(){
  ...
  $('#paymentMonthOut').textContent=money(monthRows.filter(p=>['supplier_payment','expense'].includes(p.payment_type)).reduce((sum,p)=>sum+Number(p.amount||0),0));
...
function paymentCustomerOptions(selected=''){
  const projects=state.projects.filter(p=>p.status==='Approved'&&projectMetrics(p).balance>0),orders=state.walkInOrders.filter(o=>o.status!=='Cancelled'&&walkInMetrics(o).balance>0);
...
$('#editPaymentAccountBtn').addEventListener('click',()=>openPaymentAccount(state.activePaymentAccount?.id));$('#accountMoneyInBtn').addEventListener('click',()=>openPayment('income',{accountId:state.activePaymentAccount?.id}));$('#accountMoneyOutBtn').addEventListener('click',()=>openPayment('expense',{accountId:state.activePaymentAccount?.id}));$('#accountTransferBtn').addEventListener('click',()=>openPayment('transfer',{accountId:state.activePaymentAccount?.id}));$('#receiveInvoicePaymentBtn').addEventListener('click',()=>openPayment('customer_receipt',{projectId:state.activeInvoice?.project_id}));$('#printPaymentBtn').addEventListener('click',()=>printSheet('#paymentDetailSheet'));
