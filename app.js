...
function documentHtml(q){
  const p=state.parties.find(x=>x.id===q.party_id)||{}; const price=q.pricing_mode==='both'?`<div><small>WITH MATERIAL</small><b>${money(q.price_with_material)}</b></div><div><small>LABOUR ONLY</small><b>${money(q.price_without_material)}</b></div>`:`<span>${q.pricing_mode==='with_material'?'WITH MATERIAL':'LABOUR ONLY'}</span><b>${money(quoteAmount(q))}</b>`;
