...
function setReportFilterVisibility(){
  const shown={projectbilling:['From','To','Project','Party'],projectprofit:['From','To','Project','Party'],materialusage:['From','To'],labour:['From','To','Project'],expenses:['From','To','Project','Account'],profitability:['From','To','Project','Party'],cashflow:['From','To','Project','Party','Supplier','Account'],receivables:['From','To','Project','Party'],payables:['From','To','Supplier']}[reportTab];
...
function labourReportRows(){const target=$('#reportProjectFilter').value;return state.labourers.map(l=>{const assignments=state.labourAssignments.filter(a=>a.labourer_id===l.id&&reportDateOk(a.assignment_date)&&(target==='all'||target===(a.project_id?`project:${a.project_id}`:`walkin:${a.walk_in_order_id}`))),payments=state.payments.filter(p=>p.labourer_id===l.id&&['labour_payment','labour_advance'].includes(p.payment_type)&&reportDateOk(p.payment_date)),accrued=assignments.reduce((s,a)=>s+reportNum(a.amount??reportNum(a.days)*reportNum(a.daily_rate)),0),paid=payments.filter(p=>p.payment_type==='labour_payment').reduce((s,p)=>s+reportNum(p.amount),0),advance=payments.filter(p=>p.payment_type==='labour_advance').reduce((s,p)=>s+reportNum(p.amount),0);return{labourer:l,accrued,paid,advance,due:accrued-paid-advance}}).filter(r=>r.accrued||r.paid||r.advance)}
function renderLabourReport(){const rows=labourReportRows(),accrued=rows.reduce((s,r)=>s+r.accrued,0),paid=rows.reduce((s,r)=>s+r.paid,0),advance=rows.reduce((s,r)=>s+r.advance,0),due=accrued-paid-advance;$('#reportSummary').innerHTML=reportSummaryCard('Wages accrued',money(accrued),`${rows.length} labour person${rows.length===1?'':'s'}`,'primary')+reportSummaryCard('Wages paid',money(paid),'Payments in selected period','good')+reportSummaryCard('Advances',money(advance),'Advances in selected period')+reportSummaryCard('Net payable',money(due),'Accrued less paid and advances',due<0?'negative':'good');$('#reportContent').innerHTML=rows.length?`<div class="report-section-title"><div><p class="eyebrow">LABOUR REPORT</p><h2>Accrued, paid and advances</h2></div></div><table class="report-two-column"><thead><tr><th>Labour person</th><th>Accrued</th><th>Paid</th><th>Advance</th><th>Payable</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${esc(r.labourer.name)}</b><small>${money(r.labourer.daily_wage_rate)}/day</small></td><td>${money(r.accrued)}</td><td>${money(r.paid)}</td><td>${money(r.advance)}</td><td>${money(r.due)}</td></tr>`).join('')}</tbody></table>`:reportEmpty('No labour activity','Assign wages or record labour payments in this period.')}

function expenseReportRows(){const target=$('#reportProjectFilter').value,account=$('#reportAccountFilter').value;return state.payments.filter(p=>p.payment_type==='expense'&&reportDateOk(p.payment_date)&&(account==='all'||p.from_account_id===account)&&(target==='all'||target===(p.project_id?`project:${p.project_id}`:p.walk_in_order_id?`walkin:${p.walk_in_order_id}`:''))).sort((a,b)=>String(b.payment_date).localeCompare(String(a.payment_date)))}
function renderExpenseReport(){const rows=expenseReportRows(),general=rows.filter(p=>!p.project_id&&!p.walk_in_order_id).reduce((s,p)=>s+reportNum(p.amount),0),projects=rows.filter(p=>p.project_id).reduce((s,p)=>s+reportNum(p.amount),0),orders=rows.filter(p=>p.walk_in_order_id).reduce((s,p)=>s+reportNum(p.amount),0);$('#reportSummary').innerHTML=reportSummaryCard('Total expenses',money(general+projects+orders),`${rows.length} expense${rows.length===1?'':'s'}`,'primary')+reportSummaryCard('General',money(general),'Business expenses')+reportSummaryCard('Projects',money(projects),'Assigned project expenses')+reportSummaryCard('Orders',money(orders),'Assigned order expenses');$('#reportContent').innerHTML=rows.length?`<div class="report-section-title"><div><p class="eyebrow">EXPENSE REPORT</p><h2>General and job expenses</h2></div></div><div class="report-transaction-list">${rows.map(paymentCard).join('')}</div>`:reportEmpty('No expenses','No expenses match the selected filters.')}

function renderCashflow(){
  const rows=reportPaymentRows(),accountId=$('#reportAccountFilter').value;
  let moneyIn=0,moneyOut=0;
  rows.forEach(p=>{const amount=reportNum(p.amount);if(accountId!=='all'){if(p.to_account_id===accountId)moneyIn+=amount;if(p.from_account_id===accountId)moneyOut+=amount}else{if(['customer_receipt','income'].includes(p.payment_type))moneyIn+=amount;if(['supplier_payment','expense','labour_payment','labour_advance'].includes(p.payment_type))moneyOut+=amount}});
...
function reportCsvRows(){
  ...
  if(reportTab==='labour')return [['Labour person','Daily wage','Accrued','Paid','Advance','Payable'],...labourReportRows().map(r=>[r.labourer.name,r.labourer.daily_wage_rate,r.accrued,r.paid,r.advance,r.due])];
  if(reportTab==='expenses')return [['Number','Date','Description','Target','Account','Amount'],...expenseReportRows().map(p=>[p.payment_number,p.payment_date,p.description,paymentTargetName(p),paymentAccountName(p.from_account_id),p.amount])];
...
function prepareCurrentReport(){
  renderReports();const title={projectbilling:'Project invoices and receipts',projectprofit:'Project profitability report',materialusage:'Material purchases and assignments',labour:'Labour wages and payments report',expenses:'Expense report',profitability:'Profitability report',cashflow:'Cash flow report',receivables:'Customer receivables report',payables:'Supplier payables report'}[reportTab],from=$('#reportFrom').value,to=$('#reportTo').value,range=from||to?`${from?prettyDate(from):'Beginning'} to ${to?prettyDate(to):'Today'}`:'All time';
